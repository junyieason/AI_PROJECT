...
def createTeam(firstIndex, secondIndex, isRed,
               first='q_attack_agent', second='Semi_Attack_Defence_Agent'):
    """
...
class Attack_Agent(CaptureAgent):
    ...
    def cal_enemy_with_in_1step(self,gameState, enemy_position_list):
        min_length = 9999
        ...
        for pos in enemy_position_list:
            temp_length = util.manhattanDistance(my_pos, pos)
            if(temp_length <= 1):
                enemy_posotion_list.append(pos)
        return enemy_posotion_list
            ...
            # at opponent mode
                ...
                if pro_time >= 2:
                    ...
                    if not current_target_is_food or current_target is None:
                        ...
                        #     self.current_target = self.getClosestPos(gameState, food_list)
                        if current_food_remaining > 2 and self.current_target is None:
                            self.current_target = self.getClosestPos(gameState, food_list)
                ...
                # when the pacman  used the capsule  and no time left (avoid ghost)
                    ...
                    enemy_1step_list = self.cal_enemy_with_in_1step(gameState,enemy_location_list)
                    # 当附 enemy who 距离pacman为1 的个数大于1---》 开始逃跑回基地
                    if(len(enemy_1step_list) >= 1):
                        # self.current_target = self.getClosestPos(gameState, self.boundary)
    ...
    def getFarestPos(self, gameState, pos_list):
        max_length = -9999
        max_pos = None
        my_local_state = gameState.getAgentState(self.index)
        my_pos = my_local_state.getPosition()
...
class PositionSearchProblem:
    ...
    def __init__(self, gameState, goal, agentIndex=0, enemy_position_list=[], costFn=lambda x: 1):
        self.enemy_location_list = enemy_position_list
    ...
    def getSuccessors(self, state):
        successors = []
        choosed_enemy_posList = []
        enemy_position = self.enemy_location_list
        if(enemy_position != []):
            choosed_enemy_posList = self.cal_enemy_with_in_2step( enemy_position)


        if(choosed_enemy_posList == []):
            action_list = [game.Directions.NORTH, game.Directions.SOUTH, game.Directions.EAST, game.Directions.WEST]
        else:
            action_list = self.pure_action_list(choosed_enemy_posList)
    ...
    ### 紧用有敌人的方向
    def getClosest_distance(self, my_pos, pos_list):
        min_length = 9999
        for pos in pos_list:
            temp_length = self.getMazeDistance(my_pos, pos)
    ...
    ### 对于不同方向action 计算执行当前action之后agent 距离敌人的最小距离，（action,距离）
    #### 去除距离敌人最近的那个action
    def pure_action_list(self,enemy_location_list):
        action_list = [game.Directions.NORTH, game.Directions.SOUTH, game.Directions.EAST, game.Directions.WEST]
        distance_after_action = self.cal_distance_list_byAction(enemy_location_list,action_list)
        min_distance_af_action = min(distance_after_action)
        return_action_list = []
        for i in range(4):
            distance = distance_after_action[i]
            if distance != min_distance_af_action:
                print(i)
                print("len action list")
                print(len(action_list))
                return_action_list.append(action_list[i])
        return return_action_list

    def cal_distance_list_byAction(self,enemy_location_list,action_list):
        result_list = []
        x, y = self.getStartState()
        for i in range(len(action_list)):
            current_action = action_list[i]
            dx, dy = game.Actions.directionToVector(current_action)
            nextx_NP, nexty_NP = int(x + dx), int(y + dy)
            my_pos = (nextx_NP, nexty_NP)
            distance_to_enemy_after = self.getClosest_distance(my_pos,enemy_location_list)
            result_list.append(distance_to_enemy_after)
        return result_list


    def cal_enemy_with_in_2step(self, enemy_position_list):
        min_length = 9999
        enemy_posotion_list = []
        x, y = self.getStartState()
        my_pos = (int(x), int(y))
    ...
    def getMazeDistance(self, my_pos, pos):
        pass
